...
public class TodoRest {
    ...
    @PutMapping("/{cardId}")
    public Card updateCard(@PathVariable String cardId, @RequestBody CardRequest request) throws ValidateException {
    ...
    @PatchMapping("/{cardId}")
    public Card patchCard(@PathVariable("cardId") String cardId, @RequestBody Map<String, String> request) throws ValidateException {
        Card card = cardManager.getCardById(cardId).orElse(null);
        if (card == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }
    ...
    public DeleteInfo deleteCard(@PathVariable String cardId) {
